...
import {CommandUtil, IEnvelope, IPostal, SafeJson} from 'firmament-yargs';
...
import {LoopBackApplication2, WebSocketConn} from '../../custom-typings';
import {Url} from 'url';
...
export class WebSocketServiceImpl implements WebSocketService {
  ...
  initSubscriptions(cb: (err: Error, result: any) => void) {
    ...
    me.server.on('started', () => {
      const io = require('socket.io')(me.server.http);
    ...
    private createWebSocketServerAndStartListening(port: number) {
      ...
      wsServer.on('connection', conn => {
        ...
        conn.on('text', text => {
          ...
          me.safeJson.safeParse(text, (err: Error, envelope: IEnvelope<any>) => {
            if (err) {
